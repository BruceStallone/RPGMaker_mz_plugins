 ...
 * @help  
 实现项目图鉴。
 *这个插件可以自由设定物品、武器、防具的显示内容。
 *
 *设置页面上的每个项目
 *各页的项目从“项目项目设定”、“武器项目设定”、“防具项目设定”中设定。
 *可以在项目列表旁边的（）数字项中设置每个项目的设置。
 * 
 *【名称设置】
 *您可以设置项目名称以外的任何名称。
 *如果未填写，则显示数据库以及在此插件中设置的名称。
 * 
 *【系统项目文字颜色的设置】
 *指定名称的字符颜色。
 *
 ...
 *
 *【单位】
 *设置用于结尾的单位。可以设置原始参数、成功率。
 * 
 *【内容背景显示】
 *显示黑色背景（默认情况下除外），并设置是否显示表格。
 *能力值、持有数、最大持有数、价格、原创参数、可使用时、消耗、速度修正率、得TP、武器类型、防具类型、装备类型均可设定。
 *
 *【记述栏、个别指定图像标签名】
 *请填写任意字符串。某些字符串可能不可用，因此在这种情况下，可能会显示为“”。
 *物品、武器、防具的笔记栏
 *<文本>描述栏中的文本
 *：描述栏标记名称：要显示的文本。如果换行的话，几行都可以显示，所以也可以添加自己的项目。
 ...
 *
 *【图像的最大纵宽】
 *将图像的显示大小调整为指定行的大小。因为默认设置为8行，所以超过8行的高度后调整大小。
 *可以在公共图像、项目个别图像中设定。
 * 
 *显示物品、武器、防具的个别图像
 *物品、武器、防具的笔记栏
 *<[img]、[x]、[y]>项目个别图像的显示
 *[tag]：描述栏，用个别指定图像标签名设定的标签名[img]：图像路径（无扩展名）
 *如果单独指定的图像文件夹是“pictures”，请在没有扩展名的情况下填写img/pictures正下方的文件。
 *如果要从子文件夹中检索，请填写子文件夹名称。例'items/tankobu'
 *[x]：x坐标（相对）
 *“y”：y坐标（相对）
 *想要指定多个图像时，请只设定在项目列表中显示的部分，用记述栏、个别指定图像标签名用不同的名字设定。
 * 
 *另外，项目图像也可以从插件参数的项目图像设定、武器图像设定、防具图像设定中设定。
 *此时，如果在项目设定中选择了个别指定图像（101），则显示图像。
 *如果已填写上述标记，则标记优先。
 *
 *为项目设置自己的类别
 *物品、武器、防具的笔记栏
 *<[categorytag]:[categorykey]>填写要显示的项目类别。
 *[categorytag]:在“类别标记名称”中设置的标记名称
 *[categorykey]：类别键
 *allItem:所有项目，包括关键项目
 *allItems:所有物品、武器、防具
 *字符的开头和结尾“和”不加也没有问题，但为了慎重起见，有些字符串可能不会反映出来。
 *请不要在填写备注栏的标签时添加。
 * 
 ...
 * 2021/11/3 Ver.1.4.0
 * 完成率ウィンドウに完成率（アイテム、武器、防具）、情報登録数/所有项目、武器、防具を表示する機能を追加。
 * 2021/9/28 Ver.1.3.0
 * プラグインコマンドに図鑑完成度、情報登録済みアイテム数を变量に代入する機能を追加。
 ...
 * @desc 将图鉴信息登录的所有物品（包括武器、防具）的数量代入变量。
 * @text 图鉴登録数取得
 * 
 * @arg VariableId
 * @type variable
 ...
 * 
 * @param UnknownData
 * @desc 未确认的索引名。？只加一个字的话，根据名字的字数？将条目添加到文档注册表
 * @text 未确认项目名称
 * @type string
 * @default ？
 * @parent BasicSetting
 ...
 * @param DecimalMode
 * @text 端数处理四舍五入
 * @desc 四舍五入舍入舍入显示外小数点。（用false截断）
 * @type boolean
 * @default true
 * @parent BasicSetting
 * 
 * @param PageNextSymbol
 * @desc 进页符号名称（要更改，需要一个可以单独分配密钥的插件）
 * @text 进页符号名称
 * @type combo
 * @option 
 * @option pageup2
 ...
 * @desc 页面返回符号名称（要变更的话需要可以另外分配密钥的插件）
 * @text 页面返回符号名称
 * @type combo
 * @option 
 * @option pagedown2
...
* @text 参数表达式(1)
* @type combo
* @option '$gameVariables.value(0);//ゲーム变量'
* @default 
* @parent BasicSetting
* 
* @param NameColor
* @desc 系统项的字符颜色。可以在文本选项卡中输入颜色代码。
* @text 系统项目文字颜色(2)
* @type number
* @default 16
* @min 0
...
* 
* @param textMethod
* @desc 記述欄、画像（個別指定画像）标记名称
* @text 記述欄、个别指定图像标签名称(12)
* @type string
* @default 
* @parent textSetting
* 
...
* @option 図鑑防具完成率
* @value 3
* @option 図鑑登録数(登録数/所有项目、武器、防具)
* @value 10
* @option 图鉴项目登录数(登録数/所有项目)
* @value 11
...
Window_ItemBook.prototype.paramNameShow = function(list, item) {
   ...
   switch (params) {
       ...
       case 75:
           return this.language_Jp ? "TP充电率" : 'TP Charge';
       case 76:
...
Window_ItemBook.prototype.equipType = function(list, item, x, y, width) {
 ...
 width = this.contensWidth(width);
 this.changeTextColor(this.getColorCode(list.NameColor));
 const nameText = list.paramName ? list.paramName : (this.language_Jp ? "装备类型" : "Equipment type");
 const textWidth = this.systemWidth(list.SystemItemWidth, width);
 this.drawText(nameText, x, y, textWidth);
 this.resetTextColor();
 let text;
